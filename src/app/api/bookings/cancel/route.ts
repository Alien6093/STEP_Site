...
export async function POST(request: NextRequest) {
  try {
    ...
    // resolvedMentorEmail takes priority; fall back to the full adminEmails list
    // so all admins receive the notification (dual-routing).
    const recipientEmails: string[] =
      resolvedMentorEmail ? [resolvedMentorEmail] : adminEmails;

    /* ── Edge case: no recipients configured → skip send, log warning ──── */
    if (recipientEmails.length === 0) {
      console.warn(
        "[bookings/cancel] ADMIN_EMAILS env var is empty and no mentor email resolved. " +
        "Skipping cancellation notification to prevent Resend API crash."
      );
    } else {
      resend.emails
        .send({
