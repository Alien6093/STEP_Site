...
/* ─── Derive unique SECTORS and STAGES from the Sanity-sourced data ──── */

function buildOptions(startups: Startup[]) {
...
export default function PortfolioGrid({ startups }: { startups: Startup[] }) {
  const [searchTerm,     setSearchTerm]     = useState("");
  const [selectedSector, setSelectedSector] = useState("All Sectors");
  const [selectedStage,  setSelectedStage]  = useState("All Stages");

  const { sectors, stages } = useMemo(() => buildOptions(startups), [startups]);
  ...
  }, [searchTerm, selectedSector, selectedStage, startups]);
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <ScrollFadeIn>
        <FilterBar
          ...
          /* Pass dynamic options derived from Sanity data */
          sectors={sectors}
          stages={stages}
        />
      </ScrollFadeIn>
      ...
      {filteredStartups.length > 0 ? (
        ...
        >
          ...
          <h3 className="text-lg font-semibold text-slate-800 mb-2">No startups found</h3>
          <p className="text-sm text-slate-500 max-w-xs">
            No startups match your current filters. Try adjusting your search term, sector, or stage.
          </p>
          <button
            onClick={() => {
              setSearchTerm("");
