...
export const mentorSchema = defineType({
  ...
  fields: [
    ...
    defineField({
      name: "email",
      title: "Mentor Private Email",
      type: "string",
      description: "CRITICAL: Keep this private. Used ONLY for backend booking notifications. Will not be shown on the website.",
      validation: (Rule) => Rule.required().email().error('A valid email is required for booking notifications.'),
    }),
    /* ─────────────────────────────────── */
